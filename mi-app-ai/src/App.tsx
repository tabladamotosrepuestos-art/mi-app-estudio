    ...
    if (fichaRef.current) {
      ...
      canvas.toBlob(async (blob) => {
        if (blob) {
          ...
          } catch (err) { alert("Error al copiar imagen."); }
        }
      }, 'image/png');
    }
  };
...
export default function App() {
  ...
  // Lógica de carga de fotos (Manual + Arrastrar)
  const manejarArchivosFotos = (files: FileList | null) => {
    if (!files) return;
    Array.from(files).forEach(f => {
      if (f.type.startsWith('image/')) {
        const r = new FileReader();
        r.onloadend = () => {
          const skuKey = f.name.split('.')[0].toLowerCase().trim();
          setBancoFotos(prev => ({ ...prev, [skuKey]: r.result as string }));
        };
        r.readAsDataURL(f);
      }
    });
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: '#f4f7f9', fontFamily: 'sans-serif' }}>
      <aside style={{ width: '340px', background: 'white', padding: '30px', borderRight: '1px solid #ddd', overflowY: 'auto' }}>
        <h2 style={{ color: '#d90429', fontWeight: '900', marginBottom: '25px' }}>STUDIO IA</h2>
        
        {/* Contacto Portada */}
        <div style={{ background: '#f0f4f8', padding: '15px', borderRadius: '15px', marginBottom: '20px' }}>
          <p style={{ fontSize: '10px', fontWeight: 'bold', color: '#555' }}>CONTACTO EN PDF</p>
          <input type="text" value={whatsapp} onChange={e => setWhatsapp(e.target.value)} placeholder="WhatsApp" style={{ width: '100%', marginBottom: '5px', padding: '5px' }} />
          <input type="text" value={instagram} onChange={e => setInstagram(e.target.value)} placeholder="Instagram" style={{ width: '100%', padding: '5px' }} />
        </div>

        <button onClick={generarPDF} style={{ width: '100%', padding: '15px', background: 'black', color: 'white', borderRadius: '15px', fontWeight: 'bold', marginBottom: '20px', cursor: 'pointer', border: 'none' }}>📑 GENERAR CATÁLOGO (6xHoja)</button>

        {/* Reglas de Descuento */}
        <div style={{ marginBottom: '20px', background: '#f9f9f9', padding: '15px', borderRadius: '15px' }}>
          <p style={{ fontSize: '10px', color: '#aaa', fontWeight: 'bold' }}>REGLAS %</p>
        ...
        {/* Carga de Logos y Excel */}
        <div style={{ fontSize: '11px', color: '#666' }}>
          <p>LOGO EMPRESA (Portada/Ficha)</p>
          ...
          <p style={{ marginTop: '10px' }}>LOGO MARCA (Esquina)</p>
          ...
          <p style={{ marginTop: '10px' }}>EXCEL DE PRECIOS</p>
        ...
        {/* BOTÓN CARGAR FOTOS (CON DRAG & DROP) */}
        <div 
          onDragOver={(e) => { e.preventDefault(); setIsDragging(true); }}
          onDragLeave={() => setIsDragging(false)}
          onDrop={(e) => { e.preventDefault(); setIsDragging(false); manejarArchivosFotos(e.dataTransfer.files); }}
          style={{
            marginTop: '25px', padding: '25px', background: isDragging ? '#ffebee' : '#d90429', color: 'white', borderRadius: '20px', textAlign: 'center', cursor: 'pointer', border: isDragging ? '2px dashed white' : 'none', transition: '0.3s'
          }}
        >
          <label style={{ cursor: 'pointer' }}>
            {isDragging ? "¡SUELTA LAS FOTOS!" : "📷 CARGAR O ARRASTRAR FOTOS"}
            <input type="file" hidden multiple onChange={(e) => manejarArchivosFotos(e.target.files)} />
          </label>
        </div>
        
        <button onClick={() => setItems([])} style={{ width: '100%', marginTop: '20px', padding: '10px', background: '#eee', border: 'none', borderRadius: '10px', fontWeight: 'bold', cursor: 'pointer' }}>🗑️ LIMPIAR PANTALLA</button>
      </aside>

      <main style={{ flex: 1, padding: '40px', overflowY: 'auto' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto 40px' }}>
          <textarea value={skuInput} onChange={(e) => setSkuInput(e.target.value)} placeholder="Escribí o pegá SKUs aquí..." style={{ width: '100%', height: '100px', borderRadius: '20px', padding: '20px', border: '1px solid #ddd', marginBottom: '15px', fontSize: '16px' }} />
          <button onClick={() => { skuInput.split('\n').forEach(procesarSku); setSkuInput(""); }} style={{ width: '100%', padding: '18px', background: '#d90429', color: 'white', borderRadius: '15px', fontWeight: 'bold', border: 'none', cursor: 'pointer', fontSize: '16px' }}>GENERAR FICHAS</button>
        </div>
